...
// Helper to fetch all records in chunks of 1000 to bypass Supabase/PostgREST default limits
async function fetchAllChunked(tableName, selectQuery = '*') {
  let allData = [];
  let from = 0;
  let to = 999;
  let hasMore = true;
  ...
  while (hasMore) {
    ...
    if (error) {
      console.error(`Error chunk-fetching ${tableName}:`, error);
      throw error;
    }
...
export const db = {
  ...
  // Force clear cache
  clearCache() {
    memCache.liveScores = null;
    memCache.officials = null;
    memCache.adminLogs = null;
  },

  // Fetch all live ratings and profiles
  async getLiveScores(force = false) {
    if (memCache.liveScores && !force) return memCache.liveScores;
    if (isConfigured) {
      try {
        const data = await fetchAllChunked('view_positions_live_scores');
        memCache.liveScores = data;
        return data;
      } catch (error) {
    ...
    return local.positions.map(p => {
      ...
      return {
        ...
        image_url: official ? official.image_url : null, // Newly exposed!
  ...
  async getGeoRegions() {
    if (isConfigured) {
      try {
        return await fetchAllChunked('geographic_regions');
      } catch (error) {
        console.warn('getGeoRegions fetch failed:', error);
      }
    }
  ...
  async getDepartments() {
    if (isConfigured) {
      try {
        return await fetchAllChunked('departments');
      } catch (error) {
        console.warn('getDepartments fetch failed:', error);
      }
    }
    return getLocalData().departments;
  },

  async getOfficials(force = false) {
    if (memCache.officials && !force) return memCache.officials;
    if (isConfigured) {
      try {
        const data = await fetchAllChunked('officials');
        memCache.officials = data;
        return data;
      } catch (error) {
        console.warn('getOfficials fetch failed:', error);
      }
    }
    const local = getLocalData();
    memCache.officials = local.officials;
    return local.officials;
  },

  async getPositions() {
    if (isConfigured) {
      try {
        return await fetchAllChunked('positions');
      } catch (error) {
        console.warn('getPositions fetch failed:', error);
      }
    }
  ...
  async getAdminLogs(force = false) {
    if (memCache.adminLogs && !force) return memCache.adminLogs;
    if (isConfigured) {
      const { data } = await supabase
  ...
  async addOfficial(payload) {
    if (isConfigured) {
      try {
        ...
        if (memCache.officials) memCache.officials.push(data);
  ...
  async updateOfficial(id, payload) {
    if (isConfigured) {
      try {
        const { data, error } = await supabase
          .from('officials')
          ...
          .single();
        if (error) return { success: false, error: error.message };
        if (memCache.officials) {
          const idx = memCache.officials.findIndex(o => o.id === id);
          if (idx !== -1) memCache.officials[idx] = data;
        }
        if (memCache.liveScores) {
          memCache.liveScores = memCache.liveScores.map(p => 
            p.current_official_id === id ? { ...p, current_official_name: `${data.first_name} ${data.last_name}`, service_cadre: data.service_cadre } : p
          );
        }
        return { success: true, data };
      } catch (e) {
  ...
  async deleteOfficial(id) {
    if (isConfigured) {
      try {
        await supabase.from('positions').update({ current_official_id: null }).eq('current_official_id', id);
        const { error } = await supabase.from('officials').delete().eq('id', id);
        if (error) return { success: false, error: error.message };
        if (memCache.officials) memCache.officials = memCache.officials.filter(o => o.id !== id);
        if (memCache.liveScores) memCache.liveScores = memCache.liveScores.map(p => p.current_official_id === id ? { ...p, current_official_id: null, current_official_name: '' } : p);
        return { success: true };
      } catch (e) {
        return { success: false, error: e.message };
      }
    ...
    if (isConfigured) {
      try {
        const { data, error } = await supabase
          ...
          .single();
        if (error) return { success: false, error: error.message };
        this.clearCache(); // Adding a new position requires fetching views to get department/region names
        return { success: true, data };
      } catch (e) {
        return { success: false, error: e.message };
      }
    ...
    if (isConfigured) {
      try {
        const { data, error } = await supabase
          ...
          .single();
        if (error) return { success: false, error: error.message };
        this.clearCache(); // Updating a position might change department, fetch is safer
        return { success: true, data };
      } catch (e) {
        return { success: false, error: e.message };
      }
  ...
  async deletePosition(id) {
    if (isConfigured) {
      try {
        ...
        if (error) return { success: false, error: error.message };
        if (memCache.liveScores) memCache.liveScores = memCache.liveScores.filter(p => p.position_id !== id);
        return { success: true };
      } catch (e) {
        return { success: false, error: e.message };
      }
