...
Based on your knowledge and any available information about this public servant, provide an objective, evidence-based assessment.
...
You MUST respond with ONLY valid JSON (no markdown, no code fences). Use this exact schema:
{
  ...
  "analysis_summary": "<2-4 sentence objective analysis citing specific findings>",
  "sources": ["<url1>", "<url2>"]
}

IMPORTANT:
- Be objective and factual. Do not make up scandals or achievements.
- If you have no specific information, give scores in the 5.0-6.0 range and state "Limited public information available."
...
export async function runAgentForPosition(position) {
  ...
  if (!OPENROUTER_API_KEY || OPENROUTER_API_KEY.trim() === '') {
    return {
      success: false,
      error: 'OPENROUTER_API_KEY is not configured. Add your key to .env.local'
    };
  }
  ...
  try {
    ...
    // 2. Build search query (used as context in prompt since OpenRouter doesn't natively browse)
    ...
    const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      headers: {
        ...
        "HTTP-Referer": "https://sarkardada.com", 
        "X-Title": "Sarkardada Agent",
    ...
    const summary = typeof parsed.analysis_summary === 'string' ? parsed.analysis_summary : 'Analysis completed.';
